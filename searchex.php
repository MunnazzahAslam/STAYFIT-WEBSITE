...
$s="SELECT * FROM exercise
            WHERE (`exercise_name` LIKE '%".$query."%')" or die(mysqli_error());
        ...
        elseif(mysqli_num_rows($result) > 0){ // if one or more rows are returned do following
            ...
            // $results = mysql_fetch_array($raw_results) puts data from database into array, while it's valid it does the loop
                ...
                echo "<h3>"."<br>"."<u>Exercise Name:</u>".$results['exercise_name']."<br>"."<u>Calories Burned:</u>".$results['calories_burned']."<br>"."<u>Time Duration:</u>".$results['time']."</h3>";
                
                // posts results gotten from database(title and text) you can also show id ($results['id'])
                echo "<iframe width='420' height='315' src=".$results['exercise_video']."></iframe>";
            }
